...
class Facebook implements ShouldQueue
{
    ...
    public array $platform_facebook;
    ...
    public function handle() {
        ...
        $this->platform_facebook = $this->workspace->setting('platform_facebook') ?? [];
        ...
        if ($this->type === 'comment') {
            ...
            $this->sendMessageToFacebook($response['content']);
        }
     ...
     */
    private function sendMessageToFacebook(string $content) {
        ...
        if (!$response->successful()) {
            Log::channel('facebook')->error('Failed to send message to Facebook', [
                'error' => $response->body(),
    ...
    private function replyToComment(string $content) {
        ...
        $commentId = $this->message->metadata['facebook']['comment_id'] ?? null;
        
        if (!$commentId) {
            Log::channel('facebook')->error('Failed to reply to comment - no comment ID found', [
        ...
        $url = "https://graph.facebook.com/v22.0/{$commentId}/comments";
        ...
        if (!$response->successful()) {
            Log::channel('facebook')->error('Failed to reply to comment', [
                'error' => $response->body(),
    ...
    protected function system(): string {
        ...
        $system[] = '- Keep in mind that this is a Facebook Messenger conversation.';
